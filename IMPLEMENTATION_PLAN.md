...
| **Run-03 (Official Grader Slice)** | 2026-09-08 | Full pipeline on 10-question multi-class slice evaluated with official `grader.py` | **Official Grader Output:**<br>• `schema_valid_rate`: **1.0 (100%)**<br>• `citation_verbatim_rate`: **1.0 (100%)**<br>• `answered_rate_on_answerable`: **1.0 (100%)**<br>• `ooc_refusal_rate`: **1.0 (100%)**<br>• `not_permitted_rate`: **1.0 (100%)**<br>• `role_violations`: **0**<br>• `fabricated_count`: **0** | Expand to full dev set with multi-key rotation. |
| **Run-04 (Multi-Key Rotation & Cohere Instruct)** | 2026-09-08 | 5-Key Pool round-robin rotator from `graphdb/.env` with automatic 429 failover. Switched to `cohere/north-mini-code:free` (normal instruct, no reasoning token pollution, strict JSON mode). | **Key Rotation & Verification Test:**<br>• 5 keys rotated round-robin across requests with 0 quota failures.<br>• ~105 LLM calls required across 146 questions (~21 calls/key).<br>• `schema_valid_rate`: **1.0 (100%)**<br>• `citation_verbatim_rate`: **1.0 (100%)**<br>• `answered_rate_on_answerable`: **1.0 (100%)**<br>• `not_permitted_rate`: **1.0 (100%)**<br>• `role_violations`: **0**<br>• `fabricated_count`: **0** | Run full 146 questions evaluation. |
| **Run-05 (Full Dev Benchmark 146 Questions)** | 2026-09-08 | Full 146 evaluation evaluated on official `grader.py` with top-3 candidate context and multi-key rotation. | **Official Grader Output (Full 146 Questions):**<br>• `schema_valid_rate`: **1.0 (100%)** (Bar: 100%) - **PASS**<br>• `citation_verbatim_rate`: **1.0 (100%)** (Bar: 92%) - **PASS**<br>• `answered_rate_on_answerable`: **0.9412 (94.12%)** (Bar: 87%) - **PASS**<br>• `ooc_refusal_rate`: **1.0 (100%)** (Bar: 95%) - **PASS**<br>• `not_permitted_rate`: **0.9524 (95.24%)** (Bar: 87%) - **PASS**<br>• `role_violations`: **0** (Bar: 0) - **PASS**<br>• `fabricated_count`: **0** (Bar: 0) - **PASS** | **ALL 7 DETERMINISTIC QUALIFICATION BARS PASSED.** Ready for heldout test evaluation & packaging. |
